...
namespace ImGui
{
    ...
    IMGUI_API ImVec2        GetWindowPos();                             // get current window position in screen space (IT IS UNLIKELY YOU EVER NEED TO USE  . Consider always using GetCursorScreenPos() and GetContentRegionAvail() instead)
    IMGUI_API ImVec2        GetWindowSize();                            // get current window size (IT IS UNLIKELY YOU EVER NEED TO USE  . Consider always using GetCursorScreenPos() and GetContentRegionAvail() instead)
    IMGUI_API float         GetWindowWidth();                           // get current window width (IT IS UNLIKELY YOU EVER NEED TO USE  ). Shortcut for GetWindowSize().x.
    IMGUI_API float         GetWindowHeight();                          // get current window height (IT IS UNLIKELY YOU EVER NEED TO USE  ). Shortcut for GetWindowSize().y.
    ...
    IMGUI_API float         GetFontSize();                                                  // get current scaled font size (= height in pixels). AFTER global scale factors applied. *IMPORTANT* DO NOT PASS   VALUE TO PushFont()! Use ImGui::GetStyle().FontSizeBase to get value before global scale factors.
    ...
    IMGUI_API ImVec2        GetCursorScreenPos();                                           // cursor position, absolute coordinates.   IS YOUR BEST FRIEND (prefer using this rather than GetCursorPos(), also more useful to work with ImDrawList API).
    IMGUI_API void          SetCursorScreenPos(const ImVec2& pos);                          // cursor position, absolute coordinates.   IS YOUR BEST FRIEND.
    IMGUI_API ImVec2        GetContentRegionAvail();                                        // available space from current position.   IS YOUR BEST FRIEND.
...
// - USING   IS NOT MANDATORY. This is only a helper and not a required API.
...
struct ImFontConfig
{
    ...
    //ImVec2        GlyphExtraSpacing;      // 0, 0     // (REMOVED AT IT SEEMS LARGELY OBSOLETE. PLEASE REPORT IF YOU WERE USING  ). Extra spacing (in pixels) between glyphs when rendered: essentially add to glyph->AdvanceX. Only X axis is supported for now.
    ...
    unsigned int    FontLoaderFlags;        // 0        // Settings for custom font builder.   IS BUILDER IMPLEMENTATION DEPENDENT. Leave as zero if unsure.
...
struct ImFontAtlas
{
    ...
    unsigned int                FontLoaderFlags;    // Shared flags (for all fonts) for font loader.   IS BUILD IMPLEMENTATION DEPENDENT (e.g. Per-font override is also available in ImFontConfig).
...
// - Important:      IS NOT EASY TO DO and comes with many subtleties not described here!
